...
export class ServerKeyRepository implements IServerKeyRepository {
  ...
  async verify({
    ...
    if (payload.iss != ISSUER) {
      return [new InvalidAuthenticationError("ServerKeyRepository", "wrong issuer")];
    }

    if (![payload.aud ?? ""].flat().includes(AUDIENCE)) {
      return [new InvalidAuthenticationError("ServerKeyRepository", "wrong audience")];
    }

    if (payload.exp != null && payload.exp <= now) {
      return [new InvalidAuthenticationError("ServerKeyRepository", "expired")];
    }

    if (payload.nbf != null && now < payload.nbf) {
      return [new InvalidAuthenticationError("ServerKeyRepository", "before active")];
    }

    if (payload.sub == null) {
      return [new InvalidAuthenticationError("ServerKeyRepository", "empty subject")];
    }
